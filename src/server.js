...
io.on("connection", (socket) => {
    ...
    socket.on("joinRoom", (code) => {
        ...
        if (rooms[code]) {
            const currentPlayer = players[socket.id];
            rooms[code].gameState.players.push(currentPlayer);
    ...
    socket.on("leaveRoom", (code) => {
        ...
        if (rooms[code]) {
            rooms[code].gameState.players = rooms[code].gameState.players.filter((player) => player.id !== socket.id);
            io.to(code).emit("playerListUpdated", {
                players: rooms[code].gameState.players,
            });
    ...
    socket.on("nextTurn", (data) => {
        ...
        if (rooms[code] && rooms[code].gameState.started) {
            ...
            if (playersArr.length === 0 || !currentPlayer)
                return;
            const currentIndex = playersArr.findIndex((p) => p.id === currentPlayer.id);
